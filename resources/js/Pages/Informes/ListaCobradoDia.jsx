...
const ListaCobradoDia = (params) => {

    const glob = new GlobalFunctions()
    const [datosAbonos, setDatosAbonos] = useState(params.datosInformes.abonos)
    ...
    return (
        ...
        >
            ...
            <div className='container'>
                <div style={{ marginTop: '0.5em' }} className="row justify-content-center">
                    <div className='row col-lg-6 col-md-12 col-sm-12 col-12'>
                        <div style={{ textAlign: 'center' }} className='col-12'>
                            <span className='titulo' style={{ width: '80%' }}><strong>Abonos entre</strong></span>
                        </div>
                        <div className="col-2">
                ...
                <br></br>
                {datosAbonos.datos.length == 0 ?
                    <span style={{ marginTop: '1.5em' }} className='container'><td colSpan='6'>No se han encontrado resultados....</td></span>
                    :
                    datosAbonos.datos.map((item, index) => {
                        return (
                            <div key={index} style={{ textAlign: 'center' }} className='table-responsive'>
                                <div >
                                    <table className="table table-sm">
                                        <thead  className="navBarFondo">
                                            <tr style={{ fontStyle: 'italic' }}>
                                                <th style={{ color: 'white' }} >Total cartera {item.cartera}</th>
                                ...
                                {item.datos.length == 0 ?
                                    ...
                                    item.datos.map((item1, index) => {
                                        return (
                                            <table key={index} style={{ marginTop: '0.4em', display: item1.listaAbonos.length == 0 ? 'none' : '' }} className="table table-sm">
                                                <thead >
                                                    <tr className='align-middle' >
                                                        <th style={{ color: 'white' }} className="navBarFondo rounded">Fecha ingreso</th>
                                                        <th style={{ color: 'white' }} className="navBarFondo rounded">Abonos</th>
                                                ...
                                                {item1.listaAbonos.length == 0 ?
                                                    <></>
                                                    :
                                                    item1.listaAbonos.map((item2, index) => {
                                                        
                                                        return (
                                                            <tbody key={index}>
                                                                <tr className='align-middle' data-toggle="tooltip" data-placement="top" title="Guardado en historial!" style={{ backgroundColor: item2.histo ? '#a18262' : '' }} key={index}>
                                                                    <td >{item2.fingreso}</td>
                                                                    <td style={{ whiteSpace: 'nowrap' }}>$ {glob.formatNumber(item2.valor_abono)}</td>
                                                ...
                                                <thead>
                                                    <tr style={{ fontStyle: 'italic' }}>
                                                        <th style={{ whiteSpace: 'nowrap' }} scope="col">Total asesor {item1.asesor}</th>
...
export default ListaCobradoDia